...
Because `contentful-export` nests variants onto their parent rather than exporting them as a flat top-level array (`experience.optimizationVariants` / `experienceFragment.optimizationVariants` — see [contentful-export's ExO doc](https://github.com/contentful/contentful-export/blob/main/docs/exo-export.md#optimization-variants) for the full rationale), variant import is opt-in and only runs when the source data actually has them:

- **Opt-in, not a separate flag.** Variant import has no `includeExoVariants`-style toggle of its own on this side — it's driven entirely by whether `includeExperienceOrchestration: true` is set and whether the source Experience/Fragment objects carry a non-empty `optimizationVariants` array. If the export was run without `includeExoVariants: true`, the field is absent and nothing happens here.
