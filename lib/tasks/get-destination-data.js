...
export default function getDestinationData ({
  ...
  return client.getSpace(spaceId)
    ...
    .then((environment) => {
      ...
      // TODO do we even need to fetch these tbh?
      //include tags even if contentModelOnly = true
      // all tags are returned from tags collection endpoint
      environment.getTags().then((tags) => {
        result.tags = tags
      }).catch(e => {
        // make sure to catch here
        // users without access to Tags will get 404
        // if they dont have access, remove tags array so they're not imported again??? TODO this this the right thing to do
        delete result.tags
      })
