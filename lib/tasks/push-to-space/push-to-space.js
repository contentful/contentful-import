...
export default function pushToSpace ({
  ...
  return new Listr([
    ...
    },
    {
      ...
      task: wrapTask(async (ctx, task) => {
        const allPublishedContentTypes = await publishEntities(ctx, task, ctx.data.contentTypes, sourceData.contentTypes)
    ...
    },
    {
      ...
      task: wrapTask(async (ctx, task) => {
        ...
        const allProcessedAssets = await assets.processAssets(assetsToProcess, {timeout, retryLimit})
