 ...
 */
export function createEntities ({ context, entities, destinationEntities, requestQueue }) {
  return createEntitiesWithConcurrency({ context, entities, destinationEntities, requestQueue })
}

// TODO
// Locales need to be created in series
export function createLocales ({ context, entities, destinationEntities, requestQueue }) {
  return createEntitiesInSequence({ context, entities, destinationEntities, requestQueue })
}

async function createEntitiesWithConcurrency ({ context, entities, destinationEntities, requestQueue }) {
  const pendingCreatedEntities = entities.map((entity) => {
    ...
    return requestQueue.add(async () => {
      try {
        ...
        handleCreationErrors(entity, err)
        return null
      }
    })
  })
...
async function createEntitiesInSequence ({ context, entities, destinationEntities, requestQueue }) {
 ...
 */
export async function createEntries ({ context, entities, destinationEntities, requestQueue }) {
  const createdEntries = await Promise.all(entities.map((entry) => {
    return createEntry({ entry, target: context.target, skipContentModel: context.skipContentModel, destinationEntities, requestQueue })
  }))

  return createdEntries.filter((entry) => entry)
}

async function createEntry ({ entry, target, skipContentModel, destinationEntities, requestQueue }) {
  const contentTypeId = entry.original.sys.contentType.sys.id
  const destinationEntry = getDestinationEntityForSourceEntity(
    destinationEntities, entry.transformed)
  ...
  try {
    ...
    */
    if (skipContentModel && err.name === 'UnknownField') {
      ...
      return createEntry({ entry, target, skipContentModel, destinationEntities, requestQueue })
    }
    err.entity = entry
...
// TODO
// We need to pre index all our entitites by ID else large imports will spend
// a long time searching over the entities list again and again
